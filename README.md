...
- Python 3.8+
- Nginx（已在 1.20.1 上验证；HTTP/2 语法随版本不同：1.25.1+ 用 `http2 on;`，旧版本用 `listen <port> http2;`）
- curl 命令行工具
- grpcio（仅 gRPC 测试需要）
...
3. **Nginx 版本**：HTTP/2 语法与版本相关——1.25.1+ 用 `http2 on;`，旧版本（如 1.20.1）需写为 `listen <port> http2;`。当前测试数据以 1.20.1 为基线
4. **IPv6 监听**：当 `localhost` 解析优先返回 `::1` 时，测试 server 块需同时声明 `listen 80;` 与 `listen [::]:80;`，否则请求会被系统默认 server 拦截
5. **proxy_pass 目标**：测试用例的 `proxy_pass` 不要指向本机 Nginx 监听端口（如 `:80`），否则形成自循环导致 curl 超时；使用未监听端口即可
6. **日志查看**：测试运行后查看 `logs/info.log` 获取详细执行日志
7. **权限要求**：测试需要对 Nginx 配置文件和备份目录的写入权限，建议使用 sudo 或配置适当的文件权限
